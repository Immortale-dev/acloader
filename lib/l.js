(function(gr){
	...
	lProto.setSettings = function(s){
		...
		this.__calcLineTypes();
		this.__calcRotateTypes();
		this._configCanvas();
	}
	...
	lProto._addPoint = function(){
		...
		var lineLength = this._getLineWidth(lng);
	...
	lProto._rotate = function(){
		...
		var ra = this._getRotateAngle(st.rotatetype);
	...
	lProto._calc = function(){
		...
		var angle = st.angle + this._getRotateAngle(st.rotatetype)*st.coofrotate;
		...
		for(i=last;i>0;i--){
			...
			let lw = this._getLineWidth(lineType);
	...
	lProto._initProperties = function(){
		...
		var o = ["lineTypesCount", "rotateTypesCount", "anglesCount", "gapWidth", "gapTime", "gapTimeout", "maxLength", "fastRotateTime", "slowRotateTime", "shortLineTime", "longLineTime", "shortLineLength", "longLineLength", "lineWidth", "fastRotateAngle", "slowRotateAngle", "scale", "lineTypes", "rotateTypes"];
		
		var cltf = function(){
			if(!_this._disableConf)
					_this.__calcLineTypes();
		}
		var crtf = function(){
			if(!_this._disableConf)
					_this.__calcRotateTypes();
		}
		
		var fo = {
			lineTypesCount:cltf,
			shortLineLength:cltf,
			longLineLength:cltf,
			rotateTypesCount:crtf,
			fastRotateAngle:crtf,
			slowRotateAngle:crtf,
			lineTypes:function(){
				pri.linetypescount = pri.linetypes.length;
			},
			rotateTypes:function(){
				pri.rotatetypescount = pri.rotatetypes.length;
			}
		};
		...
		Object.defineProperty(pub,'shiftX',{
			get: function(){
				return pri.shift.x;
			},
			set: function(val){
		...
		Object.defineProperty(pub,'shiftY',{
			get: function(){
				return pri.shift.y;
			},
			set: function(val){
	...
	lProto._getLineWidth = function(i){
		return this._settings.linetypes[i%this._settings.linetypescount];
	}
	
	lProto._getRotateAngle = function(i){
		return this._settings.rotatetypes[i%this._settings.rotatetypescount];
	}
	...
	lProto.__calcLineTypes = function(){
		var lt = [];
		for(let i=0;i<this._settings.linetypescount;i++){
			lt.push(this.__calcLineWidth(i));
		}
		this._settings.linetypes = lt;
	}
	
	lProto.__calcRotateTypes = function(){
		var lt = [];
		for(let i=0;i<this._settings.rotatetypescount;i++){
			lt.push(this.__calcRotation(i));
		}
		this._settings.rotatetypes = lt;
	}
