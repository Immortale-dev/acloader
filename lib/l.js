(function(gr){
	...
	lProto.setSettings = function(s){
		...
		pri.color = parseColor(pub.color);
		pri.fadeColor = pub.fadeColor ? parseColor(pub.fadeColor) : pri.color.slice(0);
		pri.fadeColor[3] = 0;
		
		pri.gapColor = parseColor(pub.gapColor);
	...
	lProto._configCanvas = function(){
		...
		var cw = this.settings.scale*2*(this.__calcCanvasRadius()+sr)+10; //Advanced
		pri.canvas.width = cw;
			...
			if(!st.gaptype){
				...
				if(gow * st.coofgap > gapw){
					...
					if(gapc > 0.5)
						...
						nogs = true;
					gapc*=2;
					cg1[3] = gapc * pri.gapColor[3];
					cg2[3] = gapc * pri.gapColor[3];
				...
				if(gow * st.coofgap + gapw < gow){
					...
					if(gapc > 0.5)
						...
						noge = true;
					gapc*=2;
					cg4[3] = gapc * pri.gapColor[3];
					cg3[3] = gapc * pri.gapColor[3];
